...
export class BuilderUI {
	...
	setupFileBrowser() {
		...
		const listContainer = document.getElementById('asset-browser-content');
		...
		btnLoad.onclick = async () => {
			...
			// Temporarily hide the file browser modal so it doesn't obstruct the screenshot generation if it uses screen capture
			// However, since we use a separate canvas render, it might be fine.
	...
	// Renders the File Browser (Inside Modal)
	renderFileBrowser(assets, folderName) {
		...
		Object.keys(groups).sort().forEach(groupName => {
			...
			groups[groupName].forEach(asset => {
				...
				div.className = 'asset-item browser-item';
				...
				// Click on Browser Item -> Prompt to add to store WITHOUT closing modal
	...
	renderAssetStore() {
		...
		if (assets.length === 0) {
			container.innerHTML = '<div class="empty-state">No assets in store.<br>Add from File Browser.</div>';
			return;
		}
	...
	openAddAssetModal(file, thumb, defaultName) {
		...
		btnConfirm.onclick = () => {
			...
			this.manager.addAssetToStore(name, file, thumb).then(() => {
				close();
				// Do not close the file browser modal here
			}).catch(err => {
	...
	setupAutoSaveTimer() {
		...
		this.uiUpdateInterval = setInterval(() => {
			this.updateAutoSaveUI();
			this.updateSceneStats(); // Update stats periodically
		}, 1000);
		if (btnSaveNow) {
	...
	updateSceneStats() {
		...
		this.scene.scene.meshes.forEach(m => {
			if (!m.isDisposed()) {
				meshCount++;
				// Calculate polygon count (triangles)
				if (m.geometry) {
					polyCount += m.getTotalIndices() / 3;
				}
			}
		});
		
		statsEl.innerText = `Meshes: ${meshCount} | Poly: ${Math.floor(polyCount)}`;
	}
	...
	setupControls() {
		...
		// NEW: Full Clear Button
		document.getElementById('btnFullClear').onclick = () => {
			if (confirm('WARNING: This will clear the entire scene AND remove all loaded assets from the store.\nAre you sure?')) {
	...
	setupContextMenu() {
		...
		const deleteStoreItem = document.getElementById('ctx-delete-store-asset');
		...
		// 1. Sidebar Context Menu (Store Items Only)
		const sidebar = document.getElementById('left-sidebar');
		...
		// 2. File Browser Context Menu (Browser Items Only)
		const browserContent = document.getElementById('asset-browser-content');
		if (browserContent) {
		...
		// NEW: Delete from Store Action
		deleteStoreItem.onclick = () => {
			if (targetAssetName) {
				if (confirm(`Remove '${targetAssetName}' from store? This will delete all instances of it in the scene.`)) {
					this.manager.removeAssetFromStore(targetAssetName);
				}
				menu.style.display = 'none';
			}
		};
